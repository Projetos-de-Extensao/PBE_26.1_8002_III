...
6. **assinatura_aluno**: `true` se há indicação de que o campo de assinatura do estagiário/aluno foi preenchido (assinado).
7. **assinatura_empresa**: `true` se há indicação de que o campo de assinatura do representante da empresa foi preenchido (assinado).
8. **assinatura_faculdade**: `true` se há indicação de que o campo de assinatura da instituição de ensino foi preenchido (assinado).
9. Para campos não encontrados, retorne `null` (strings/datas) ou `false` (booleanos).
10. Não invente dados. Se não encontrou, retorne o valor padrão.
11. O texto pode vir com formatação irregular (espaços extras, quebras de linha, palavras grudadas). Interprete o conteúdo semanticamente mesmo que a formatação esteja degradada.
